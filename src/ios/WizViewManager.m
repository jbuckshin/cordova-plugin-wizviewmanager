...
static WizViewManager *wizViewManagerInstance = NULL;
...
- (void)createView:(CDVInvokedUrlCommand *)command {
    ...
    NSLog(@"[WizViewManager] ******* createView name:  %@ withOptions: %@, command.callbackId: %@", viewName, options, command.callbackId);
    ...
    WizLog(@"[WizViewManager] ******* current views... %@", wizViewList);
}
...
- (void)setLayout:(CDVInvokedUrlCommand *)command {
    ...
    // NSLog(@"[WizViewManager] ******* resizeView name:  %@ withOptions: %@", viewName, options);
...
- (void)timedRestart:(WKWebView *)theWebView {
    ...
    CDVInvokedUrlCommand *cmdLayout = [[CDVInvokedUrlCommand alloc] initWithArguments:[NSArray arrayWithObjects:@"mainView", nil] callbackId:@"" className:@"WizViewManager" methodName:@"setLayout"];
    ...
    for (int i = 0; i < [allKeys count]; i++) {
        if (![[allKeys objectAtIndex:i] isEqualToString:@"mainView"]) {
            CDVInvokedUrlCommand *cmdRemove = [[CDVInvokedUrlCommand alloc] initWithArguments:[NSArray arrayWithObjects:[allKeys objectAtIndex:i], nil] callbackId:@"" className:@"WizViewManager" methodName:@"removeView"];
